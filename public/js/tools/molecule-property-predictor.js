document.getElementById("generate-button").addEventListener("click", async () => {
    ...
    if (!content) {
        resultArea.textContent = "Please provide some reactants to predict properties.";
        return;
    }
    ...
    try {
        ...
        const response = await fetch("https://tools-api-alpha.vercel.app/api/chemical/predict-property", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({ structure: content })
        });
